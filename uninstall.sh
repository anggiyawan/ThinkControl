...
USER_HELPER="$HOME/.local/bin/thinkcontrol-helper"
...
if [ "$EUID" -eq 0 ]; then
    ...
    rm -f "/usr/libexec/thinkcontrol-helper"
    rm -f "/usr/share/polkit-1/actions/com.lenovo.thinkcontrol.policy"
    rm -f "/usr/lib/systemd/system/thinkcontrol-apply.service"
    rm -f "/usr/lib/systemd/system-sleep/thinkcontrol-sleep"
